...
import { Check, Search, AlertTriangle, Crown, ArrowUpRight, Brain, Plus, Edit, Trash, Cpu, KeyRound, ExternalLink, Server } from 'lucide-react';
import {
  ModelOption,
  SubscriptionStatus,
  STORAGE_KEY_MODEL,
...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  const renderModelOption = (opt: any, index: number) => {
    ...
    const isOllama = opt.id === 'ollama';
    const isGPT4o = opt.id === 'gpt-4o';
      ...
      <TooltipProvider key={opt.uniqueKey || `model-${opt.id}-${index}`}>
        <Tooltip>
            ...
            <div className='w-full'>
              ...
              >
                <div className="flex items-center">
                  {isOllama && (
                    <Server className="h-3.5 w-3.5 text-green-500 mr-2" />
                  )}
                  {isGPT4o && (
                    <Brain className="h-3.5 w-3.5 text-blue-500 mr-2" />
                  )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isOllama && (
                    <span className="text-xs px-1.5 py-0.5 rounded-sm bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-300 font-medium">
                      Local
                    </span>
                  )}
                  {isGPT4o && (
                  ...
                  {isRecommended && !isOllama && !isGPT4o && (
                    <span className="text-xs px-1.5 py-0.5 rounded-sm bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 font-medium">
                      Recommended
                    </span>
                  )}
          ...
          {!accessible ? (
            ...
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>Local Ollama server - uses your local models</p>
            </TooltipContent>
          ) : isGPT4o ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>OpenAI GPT-4o - Advanced multimodal AI model</p>
            </TooltipContent>
          ) : isCustom ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>Custom model</p>
          ...
          <div className="max-h-[400px] overflow-y-auto w-full">
            <div className="p-2">
              ...
              */}
              ...
              {/* 🎯 直接显示简化的模型列表（GPT-5, DeepSeek, Ollama） */}
              <div>
                {uniqueModels
                  .filter(m =>
