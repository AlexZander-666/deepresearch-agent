...
export default function ThreadPage({
  ...
  const handleStreamingToolCall = useCallback(
    (toolCall: StreamingToolCall | null) => {
      ...
      setToolCalls((prev) => {
        // Check if there's already a streaming tool with the same name
        ...
        // Add new streaming tool (even if there are completed tools with the same name)
        return [newToolCall];
      });
    ...
    async function loadData() {
      ...
      try {
        ...
        const [threadData, messagesData] = await Promise.all([
          getSharedThread(threadId).catch((err) => {
          ...
          getSharedMessages(threadId).catch((err) => {
