...
import { getSidebarProjects, getSidebarThreads, Project, Thread } from "@/lib/api";
...
export const useProjects = (
  customOptions?: Omit<
    UseQueryOptions<
      ...
      Error,
      Project[],
      ReturnType<typeof projectKeys.lists>
    >,
    'queryKey' | 'queryFn'
  >,
) =>
  createQueryHook(
    projectKeys.lists(),
    async () => {
      const data = await getSidebarProjects();
...
export const useThreads = (
  customOptions?: Omit<
    UseQueryOptions<
      ...
      Error,
      Thread[],
      ReturnType<typeof threadKeys.lists>
    >,
    'queryKey' | 'queryFn'
  >,
) =>
  createQueryHook(
    threadKeys.lists(),
    async () => {
      const data = await getSidebarThreads();
...
export const processThreadsWithProjects = (
  ...
  for (const thread of threads) {
    ...
    threadsWithProjects.push({
      threadId: thread.id || thread.thread_id, // 适配后端返回的字段名
      projectId: projectId,
      projectName: displayName,
      url: `/projects/${projectId}/thread/${thread.id || thread.thread_id}`,
      updatedAt:
