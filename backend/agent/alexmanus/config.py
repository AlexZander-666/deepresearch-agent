...
class AlexManusConfig:
    NAME = "AlexManus"
    DESCRIPTION = "AlexManus is your AI assistant with access to various tools and integrations to help you with tasks across domains."
    AVATAR = "🌞"
    AVATAR_COLOR = "#F59E0B"
...
def add_tool(tool_name: str, description: str, enabled: bool = True):
    AlexManusConfig.DEFAULT_TOOLS[tool_name] = {
        "enabled": enabled,
        "description": description
    }

def remove_tool(tool_name: str):
    if tool_name in AlexManusConfig.DEFAULT_TOOLS:
        del AlexManusConfig.DEFAULT_TOOLS[tool_name]

def enable_tool(tool_name: str):
    if tool_name in AlexManusConfig.DEFAULT_TOOLS:
        AlexManusConfig.DEFAULT_TOOLS[tool_name]["enabled"] = True

def disable_tool(tool_name: str):  
    if tool_name in AlexManusConfig.DEFAULT_TOOLS:
        AlexManusConfig.DEFAULT_TOOLS[tool_name]["enabled"] = False 
