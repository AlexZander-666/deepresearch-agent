...
class AlexManusAgentRecord:
    ...
    @classmethod
    def from_db_row(cls, row: Dict[str, Any]) -> 'AlexManusAgentRecord':
...
class AlexManusAgentRepository:
    def __init__(self, db: DBConnection = None):
        self.db = db or DBConnection()
    
    async def find_all_AlexManus_agents(self) -> List[AlexManusAgentRecord]:
        try:
            ...
            while True:
                ...
                ).eq('metadata->>is_AlexManus_default', 'true').range(offset, offset + page_size - 1).execute()
                
                if not result.data:
                    break
                
                batch_agents = [AlexManusAgentRecord.from_db_row(row) for row in result.data]
            ...
            logger.info(f"Found {len(all_agents)} existing AlexManus agents")
            return all_agents
            
        except Exception as e:
            logger.error(f"Failed to find AlexManus agents: {e}")
            raise
    
    async def find_AlexManus_agents_needing_sync(self, target_version_tag: str) -> List[AlexManusAgentRecord]:
        agents = await self.find_all_AlexManus_agents()
        return [
    ...
    async def get_agent_stats(self) -> Dict[str, Any]:
        try:
            ...
            ).eq('metadata->>is_AlexManus_default', 'true').execute()
    ...
    async def create_AlexManus_agent(
        self, 
        user_id: str,
    ) -> str:
        try:
            ...
            agent_data = {
                ...
                "model": AlexManusConfig.DEFAULT_MODEL,
                "system_prompt": AlexManusConfig.SYSTEM_PROMPT,
            ...
            if result.data:
                ...
                logger.info(f"Created minimal AlexManus agent {agent_id} for {user_id}")
                await self._create_initial_version(
                    agent_id=agent_id,
                    user_id=user_id,
                    system_prompt="[MANAGED]",
                    model=AlexManusConfig.DEFAULT_MODEL,
                    configured_mcps=AlexManusConfig.DEFAULT_MCPS,
                    custom_mcps=AlexManusConfig.DEFAULT_CUSTOM_MCPS,
                    agentpress_tools=AlexManusConfig.DEFAULT_TOOLS
                )
                return agent_id
            ...
            logger.error(f"Failed to create AlexManus agent for {user_id}: {e}")
            raise
    ...
    async def find_orphaned_AlexManus_agents(self) -> List[AlexManusAgentRecord]:
        """
        Find AlexManus agents that exist but don't have proper version records.
        These are agents that were created but the version creation process failed.
        """
        try:
            client = await self.db.client
            
            # Get all AlexManus agents
            ...
            for row in agents_result.data:
                if row['agent_id'] not in agents_with_versions:
                    orphaned_agents.append(AlexManusAgentRecord.from_db_row(row))
            
            logger.info(f"Found {len(orphaned_agents)} orphaned AlexManus agents")
            return orphaned_agents
            
        except Exception as e:
            logger.error(f"Failed to find orphaned AlexManus agents: {e}")
            raise
    ...
    async def create_version_record_for_existing_agent(
        ...
        try:
            ...
            # Build configuration exclusively from AlexManusConfig and provided unified_config
    ...
    async def _create_initial_version(
        ...
        try:
            ...
            await version_service.create_version(
                ...
                model=model,
                version_name="v1",
                change_description="Initial AlexManus agent version"
            )
            
            logger.info(f"Created initial version for AlexManus agent {agent_id}")
            
        except Exception as e:
            logger.error(f"Failed to create initial version for AlexManus agent {agent_id}: {e}")
            raise