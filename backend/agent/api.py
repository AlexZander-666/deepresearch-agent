...
async def get_agent_runs(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    for i, run in enumerate(agent_runs.data):
        logger.info(f"    {i+1}. ID: {run.get('id')}, agent_run_id: {run.get('agent_run_id')}, status: {run.get('status')}, started_at: {run.get('started_at')}, completed_at: {run.get('completed_at')}")
...
async def stream_agent_run(
    ...
    logger.info(f"agent_run data fetched successfully: {agent_run_data}")
    ...
    async def stream_generator(agent_run_data):
        ...
        try:
            ...
            # 创建两个独立的 Pub/Sub 客户端
            pubsub_response_task = asyncio.create_task(redis.create_pubsub())  # 监听Agent产生的新响应
            pubsub_control_task = asyncio.create_task(redis.create_pubsub())  # 监听用户的控制信号（停止等）
            ...
            async def listen_messages():
                # 监听Agent 调用工具、生成文本等数据
                response_reader = pubsub_response.listen()
                
                # 监听control_channel 如 complete、error、stop 等控制信号
                ...
                while not terminate_stream:
                    logger.info(f"Waiting for tasks to complete: {tasks}")
                    ...
                    for task in done:
                        try:
                            ...
                            # 接收的是Redis Pub/Sub的轻量级通知消息
                            # {
                            #    "type": "message",
                            #    "channel": "agent_run:abc123:response", 
                            #    "data": "new"  # ← 这里只是一个简单的通知信号
                            # }
                ...
                # 取消待处理的监听器任务，提升执行执行效果
                # 当有多个并发任务时，只要其中一个完成就立即处理，然后取消其他未完成的任务以节省资源，再重新启动新的监听循环。这样可以确保系统的响应性和资源利用效率。
                
                # 举例：你同时在等待两种消息：
                # tasks = [
                #     wait_for_streamer_message(),  # 等Agent说话
                #     wait_for_user_input()         # 等你自己按停止键
                # ]
                ...
                # if "Agent说话了":
                #    显示Agent内容()
                #    取消"等用户按键"的监听()  # ← 这就是cancel pending
                #    重新开始等待两种消息()

                # elif "用户按了停止":
                #     停止Agent说话()
                #     取消"等Agent说话"的监听()  # ← 这就是cancel pending
                #     结束监听循环()
         
                # 取消待处理的监听器任务，提升执行执行效果
                # 这里的并发，指的不是多个Agent任务，而是对同一个Agent的多角度监听
                ...
                try:
                    ...
                    if queue_item["type"] == "new_response":
                        ...
                        if new_responses_json:
                            ...
                            logger.info(f"New responses processed successfully, last processed index: {last_processed_index}")
                        else:
    ...
    try:
        ...
        if threads_result.data:
            ...
            for i, thread in enumerate(threads_result.data[:3]):  # 只显示前3个
                logger.info(f"{i+1}. thread_id: {thread['thread_id']}, created_at: {thread['created_at']}")
        else:
        ...
        if agent_runs_result.data:
            ...
            for i, run in enumerate(agent_runs_result.data[:3]):  # 只显示前3条
                logger.info(f"      {i+1}. ID: {run['id']}, 状态: {run.get('status', 'N/A')}, 线程: {run.get('thread_id')}")
        else:
...
async def get_thread(
    ...
    try:
        ...
        if agent_runs_result.data:
            ...
            # 打印最近的几条运行记录
            for i, run in enumerate(agent_runs_result.data[:3]):  # 只显示前3条
                logger.info(f"      {i+1}. ID: {run['id']}, status: {run.get('status', 'N/A')}, started_at: {run.get('started_at')}")
        else:
            logger.info(f"not found agent runs")
...
def _format_messages_from_table(messages):
    ...
    for msg in messages:
        try:
            ...
            if (msg.get("type") == "assistant" and 
                ...
                for mapping in tool_call_mapping:
                    ...
                    if matching_tool_call:
                        ...
                        logger.info(f"split assistant message: {deterministic_uuid} (tool: {matching_tool_call.get('function', {}).get('name', 'unknown')})")
                        logger.debug(f"split message field type check: message_id={type(deterministic_uuid)}, thread_id={type(split_message['thread_id'])}")
            ...
            logger.warning(f"skip format error message {msg.get('message_id', 'unknown')}: {e}")
            continue
    ...
    for tool_msg in tool_messages:
        try:
            ...
            if tool_call_id and tool_call_id in tool_call_to_assistant:
                ...
                logger.info(f"update tool message {tool_msg.get('message_id')} -> assistant {correct_assistant_id}")
        except Exception as e:
            logger.warning(f"update tool message assistant failed {tool_msg.get('message_id')}: {e}")
